...
TAC <- 118955 # in assessment year


#Reference points - updated 2024
Fmsy <- 0.174
Fmsy_lo <- 0.161
Fmsy_hi <- 0.174
Btrig <- Bpa <- 196402
...
save(list=c("ay","ts_yrs","ages","pg","TAC","Fmsy","Fmsy_lo","Fmsy_hi","Btrig","Bpa","Blim","Flim","Fpa","Fp.05"),file="data/init.RData")
...
png(paste0("data/Natural mortality/had.27.46a20 - Natural mortality SMS 2023 - raw vs smoothed.png"),width = 10, height = 7, units = "in", res = 600)
...
# Survey indices model run by Andrzej Jaworski
...
nm_prev <- readVPAFile(paste0("boot/data/FLR files - WGNSSK 2023/nor_had_nm.txt")) # previous NM

# replicate 2022 as 2023
nm_prev <- window(nm_prev,end=(ay))
nm_prev[,ac(c(ay-1):ay)] <- nm_prev[,ac(ay-2)] 

writeVPAFile(FLStock.=NULL, file.=paste0(sam.dir,"nm_prev.dat"), slot.="m", desc.="natural mortality",
             obj.=nm_prev,name.= st.desc)
...
#---------------------------------------------------------##
# make FLR object (data only) ####

stock.data65 <- stock.data
stock.data <- window(stock.data,start=1972)
...
source("data_explore.R") # to make plots for report
