...
runName <- "NShaddock_WGNSSK2024_Run1" 
...
## plot all
taf.png("output/ref_pts/spasmodic_recruitment_short_ts.png",width=9,height=7,units="in",res=300)
...
fifthpercBfmsy<-SIM_MSYBtrigger_segregBlim$rbp[SIM_MSYBtrigger_segregBlim$rbp$variable =="Spawning stock biomass" & SIM_MSYBtrigger_segregBlim$rbp$Ftarget==round(Fmsy_tmp) , "p05"] # CHECK VALUE!
...
if (savePlots) {
  ...
  savePlot(paste("output/ref_pts/07_",runName,"_Fp05_segregBlim_Trig_eqMSYplots.png"),type="png")
  dev.off()
}
...
if (Fmsy_tmp > refPts[,"Fp05"]) {
  ...
  Flow_newref  <- data.table(SIM_Fmsy_lower_segregBlim_Trig$rbp)[variable == 'Landings' & p50 >= lan * 0.95, Ftarget][1]
