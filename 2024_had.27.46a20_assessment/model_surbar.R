...
#                                                                            #
# Modifications:                                                             #
# 25 October 2009 - added fixed parameters, worked on improving speed        #
# 2 November 2009 - looked again at dimensions                               #
# 6 November 2009 - coded minimiser from minpack.lm library                  #
# 26 September 2010 - recoded for NS haddock example (WKADSAM)               #
# 7 December 2010 - updated to include multiple age-structured tuning series #
# 14 December 2010 - modularised code and renamed                            #
# 10 January 2012 - updated version for WGISDAA, Copenhagen                  #
# 8 February 2012 - updated version for WKROUND, Aberdeen                    #
# 30 April 2012 - updated version for WGNSSK, Copenhagen                     #
# 12 March 2014 - updated version for WKHAD                                  #
# 28 April 2017 - updated version for WGNSSK, Copenhagen                     #
# 5 February 2018 - updated version for WKNSEA, Copenhagen                   #
# 26 April 2018 - small updates for WGNSSK, Oostende                         #
# 24 April 2019 - small updates for WGNSSK, Bergen				                   #
# 15 April 2020 - updates for WGNSSK by webex	                               #
# 13 April 2020 - updates for WGNSSK (NoSh haddock)                          #  
##############################################################################-
...
y.win <- 7
if (pdf.plots){
  pdf(file = paste("output/SURBAR/surbar_output.pdf", sep = ""),
...
x.surv <- "q1q3"		# Surveys to use
x.ref <- 3			# Reference age
x.lambda <- 5		# Smoothing parameter
x.q <- 1.0			# Baseline q catchability

# Set up sensitivity parameters (if required)
# Standard: q1q3, 3, 3, 1
s.surv <- c("q1q3") # c("q1q3", "q1", "q3") # c("q1q3")  
s.ref <- c(3) # c(2,3) # c(3)
s.lambda <- c(5) # c(1,3,5) # c(3)
s.q <- c(1.0) # c(1.0,0.75,0.5) # c(1.0)
  ...
  for (i.ref in s.ref){
    for (i.lambda in s.lambda){
      for (i.q in s.q){
        ...
        # Northern Shelf haddock WGNSSK 2021
        sw.file <- "nosh_had_sw.dat"
        mat.file <- "nosh_had_mo.dat"
        survey.file <- "nosh_had_ibts.dat"
        #sw.file <- "nosh_had_sw_with_age0.dat"
        #mat.file <- "nosh_had_mo_with_age0.dat"
        #survey.file <- "nosh_had_ibts_with_age0.dat"
        zbarage.1 <- 3 
        zbarage.2 <- 5 
        ref.age <- i.ref 
        lambda <- i.lambda
        ...
        sw.temp <- read.vpa.file(paste(path, sw.file, sep = ""))
        ...
        mat.temp <- read.vpa.file(paste(path, mat.file, sep = ""))
        ...
        # Read in survey data
        idx.temp <- read.survey.file(paste(path, survey.file, sep = ""))
        numk <- idx.temp$n
        idx <- idx.temp$idx
        ...
        for (k in 1:numk){
          # x.mean <- mean(mean(x[[k]], na.rm = TRUE), na.rm = TRUE)
...
if(FALSE){
  ...
  plot(s0.list[[1]]$stock$year, s0.list[[1]]$stock$ssb, type = "l", lty = 1, col = 1,
       xlab = "", ylab = "Mean-std SSB", ylim = c(ymin,ymax))
  for (i in 2:length(s0.list))
  {
  ...
  plot(s0.list[[1]]$stock$year, s0.list[[1]]$stock$tsb, type = "l", lty = 1, col = 1,
       xlab = "", ylab = "Mean-std TSB", ylim = c(ymin,ymax))
  for (i in 2:length(s0.list))
  {
    lines(s0.list[[i]]$stock$year, s0.list[[i]]$stock$tsb, type = "l", lty = 1, col = i)
  }
  abline(h = 0, lty = 8)
  legend(x = "topleft", bty = "n", legend = s.surv, lty = 1, col = 1:length(s0.list))
  # Recruitment
  ...
  plot(s0.list[[1]]$stock$year, s0.list[[1]]$stock$rec, type = "l", lty = 1, col = 1,
       xlab = "", ylab = paste0("Mean-std recruitment at age ", a1), ylim = c(ymin,ymax))
  for (i in 2:length(s0.list))
  {
...
if(FALSE){
  ...
  abline(h = 0, lty = 8)
  legend(x = "topleft", bty = "n", legend = s.ref, lty = 1, col = 1:length(s0.list))
  # Recruitment
  ymin <- min(0,unlist(lapply(s0.list, function(wk){min(wk$stock$rec)})))
  ymax <- max(unlist(lapply(s0.list, function(wk){max(wk$stock$rec)})))
  plot(s0.list[[1]]$stock$year, s0.list[[1]]$stock$rec, type = "l", lty = 1, col = 1,
...
if(FALSE){
  ...
  abline(h = 0, lty = 8)
  legend(x = "topleft", bty = "n", legend = s.lambda, lty = 1, col = 1:length(s0.list))
  # Recruitment
  ymin <- min(0,unlist(lapply(s0.list, function(wk){min(wk$stock$rec)})))
  ymax <- max(unlist(lapply(s0.list, function(wk){max(wk$stock$rec)})))
  plot(s0.list[[1]]$stock$year, s0.list[[1]]$stock$rec, type = "l", lty = 1, col = 1,
...
if(FALSE){
  ...
  abline(h = 0, lty = 8)
  legend(x = "topleft", bty = "n", legend = s.q, lty = 1, col = 1:length(s0.list))
  # Recruitment
  ymin <- min(0,unlist(lapply(s0.list, function(wk){min(wk$stock$rec)})))
  ymax <- max(unlist(lapply(s0.list, function(wk){max(wk$stock$rec)})))
  plot(s0.list[[1]]$stock$year, s0.list[[1]]$stock$rec, type = "l", lty = 1, col = 1,
  ...
  abline(h = 0, lty = 8)
  legend(x = "topright", bty = "n", legend = s.q, lty = 1, col = 1:length(s0.list))
  mtext("Effect of catchability q for first age", side = 3, outer = TRUE, line = -1.5)
...
## save output
write.taf(stock,file="SURBAR stock summary results.csv",dir="output/SURBAR/",row.names=F)
...
if (!pdf.plots) windows(width = 10, height = 7)
plot.index.corr(wk.object = list(x[[1]][as.character(1983:(ay)),as.character(1:7)]), wk.type = "SURBAR", wk.name = "delta-GAM NS-WC Q1") # Q1
if (!pdf.plots) windows(width = 10, height = 7)
plot.index.corr(wk.object = list(x[[2]][as.character(1991:(ay-1)),as.character(1:7)]), wk.type = "SURBAR", wk.name = "delta-GAM NS-WC Q3+Q4") # Q3

#-------------------------------#
# Summary plots to pngs
pdf.plots<- F
plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "sum.line", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - stock summary.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "sum.boxplot", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - stock summary boxplot.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res[1], y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "res.line", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - residuals Q1.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res[1], y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "res.smooth", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - residuals with smoother Q1.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res[2], y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "res.line", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - residuals Q3_4.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res[2], y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "res.smooth", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - residuals with smoother Q3_4.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "params", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - parameters.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "catch.curve", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - catch curves.png"),type="png")
dev.off()

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "log.by.cohort", pdf.plots)
savePlot(filename=paste0("output/SURBAR/SURBAR - cohort consistency.png"),type="png")
dev.off()
...
mohn.z <- mohn.z/length((y2-1):(y2-nretro))
...
mohn.ssb <- mohn.ssb/length((y2-1):(y2-nretro))
...
mohn.tsb <- mohn.tsb/length((y2-1):(y2-nretro))
...
mohn.rec <- mohn.rec/length((y2-1):(y2-nretro))
